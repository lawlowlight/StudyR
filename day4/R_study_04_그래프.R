...
barplot(f.season, main =  '좋아하는 계절', col = rainbow(4), xlab = '계절',
        ylab =  '빈도수', horiz =  F, names = c('가을','봄','여름','겨울'),
        las = 2)

## 중첩 막대그래프
# 데이터 입력
age.A <- c(13709,10974,7979,5000,4250)
age.B <- c(17504,29701,36209,33947,24487)
age.C <- c(991,2195,5366,12980,19007)
ds <- rbind(age.A, age.B, age.C)
colnames(ds) <- c('1970','1990','2010','2030','2050')

ds
par(mfrow=c(1,1), mar=c(3,3,3,8)) # plot 여백조정
barplot(ds, main = '인구수 추정', col = rainbow(3), horiz = F, 
        beside = F, 
        legend.text = c('0~14세','15~64세','65세 이상'),
        args.legend = list(x='topright', bty='y', inset=c(-0.25,-0.05)))
...
par(mfrow=c(1,1), mar = c(4.5,4,3,3)) # plot 여백조정
...
## 히스토그램
par(mfrow=c(1,1), mar=c(4,4,4,3)) # Plot 여백조정정
dist <- cars[,2]
str(dist)
hist(dist, main = '제동거리', xlab =  '제동거리', ylab = '빈도수',
     border = 'blue', col = 'skyblue', las = 1, breaks = 5)
